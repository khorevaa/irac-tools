...
Процедура ВыполнитьКоманду(Знач Команда) Экспорт
	...
	УтилитаАдминистрирования  = Команда.ЗначениеОпции("rac");
	...
	УправлениеКластером = Новый УправлениеКластером1С(УтилитаАдминистрирования, СервисАдминистрирования, );
	Если ВыводОтладочнойИнформации Тогда
		УправлениеКластером.Лог().УстановитьУровень(УровниЛога.Отладка);
	КонецЕсли;

	ОписаниеКластера = ПолучитьОписаниеКластера(УправлениеКластером);
...
Функция ПолучитьОписаниеКластера(УправлениеКластером)

	ОписаниеКластера = Новый Соответствие();

	ОписаниеКластера.Вставить("СервисАдминистрирования",
	                          ПолучитьПоляОбъекта(УправлениеКластером));

	ОписаниеКластера.Вставить("Администраторы", ПолучитьСписокАдминистраторов(УправлениеКластером));
	...
	Для Каждого ТекКластер Из Кластеры Цикл
		...
		ТекОписаниеКластера.Вставить("Администраторы", ПолучитьСписокАдминистраторов(УправлениеКластером));
		...
		ТекОписаниеКластера.Вставить("СчетчикиРесурсов",
		                              ПолучитьСписокОбъектов(ТекКластер.СчетчикиРесурсов().Список()));

		ТекОписаниеКластера.Вставить("ОграниченияРесурсов",
		                             ПолучитьСписокОбъектов(ТекКластер.ОграниченияРесурсов().Список()));
